          ...
          {!isLoading && !error && games.length > 0 && (
            <div className="grid gap-[10px] w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 justify-items-center">
              {games.map((game: any) => (
                <div
                  key={game.id}
                  className="relative p-[10px] group cursor-pointer w-full max-w-[340px]"
                >
                  <img
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end px-6 py-4 bg-gradient-to-t from-black/70 to-transparent rounded-[32px] ">
                    <span className=" text-white font-bold text-lg lg:text-xl drop-shadow-lg">
                      {game.title}
                    </span>
                    <span className=" text-white text-sm font-normal line-clamp-2">
                      {game.description}
                    </span>
                  </div>
                </div>

...
export function EditSheetUppy({ open, onOpenChange, gameId }: EditSheetUppyProps) {
  ...
  const { updateGame, progress, isUploading, isCompleted, hasError, reset } = useFrontendUpload();
  ...
  // Convert our progress phases to user-friendly messages
  const getProgressMessage = (phase: string, message: string) => {
    switch (phase) {
      case 'extracting':
        return message.includes('Extracting') ? 'Extracting game files...' : 'Preparing files...';
      case 'uploading':
        return message;
      case 'finalizing':
        return 'Updating game record...';
      case 'completed':
      ...
      >
            ...
            <Form className="space-y-4">
                    ...
                    {thumbnailPreview ? (
                      <label className="relative w-36 h-36 cursor-pointer group">
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 bg-[#C026D3] text-white rounded-full w-6 h-6 flex items-center justify-center shadow hover:bg-[#a21caf] transition-colors disabled:opacity-50"
              ...
              <SheetFooter className="flex flex-row justify-between mt-6">
                <Button
                  ...
                  className="dark:bg-[#EF4444] disabled:opacity-50"
                >
                  Delete
                </Button>
                <div className="flex gap-2">
                  <SheetClose asChild>
                    <Button
                      ...
                      variant="outline"
                      disabled={isUploading}
                      className="dark:text-black dark:bg-white cursor-pointer disabled:opacity-50"
                    >
                      Cancel
                    </Button>
                  </SheetClose>
                  <Button
                    ...
                    variant="default"
                    className="bg-[#D946EF] hover:bg-accent dark:text-white cursor-pointer disabled:opacity-50"
                  >
        ...
        {(isUploading || isCompleted || hasError) && (
          <GameCreationProgress
            ...
            currentStep={getProgressMessage(progress.phase, progress.message)}

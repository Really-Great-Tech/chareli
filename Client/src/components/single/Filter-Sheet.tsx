import { Button } from "../ui/button"
import { Input } from "../ui/input"
import { Label } from "../ui/label"
import {
  Sheet,
  SheetClose,
  SheetContent,
//   SheetDescription,
//   SheetFooter,
  SheetHeader,
  SheetTitle,
  SheetTrigger,
} from "../ui/sheet"

export function FilterSheet({ children }: {children:React.ReactNode}) {
  return (
    <Sheet>
      <SheetTrigger asChild>
        {children}
      </SheetTrigger>
      <SheetContent className="font-pong dark:bg-[#0F1621]">
        <SheetHeader>
          <SheetTitle className="text-xl font-normal tracking-wider mt-6">Filter</SheetTitle>
         <div className="border border-b-gray-200"></div>
        </SheetHeader>
        <div className="grid gap-4 p-4">
            {/* category */}
          <div className="tems-center gap-4">
            <div className="flex flex-col space-y-2">
            <Label htmlFor="name" className="text-right text-lg">
              Select Category
            </Label>
            <Input id="name" placeholder="Select" className="col-span-3 shadow-none text-gray-400 font-thin text-sm tracking-wider font-pincuk h-14 bg-[#F1F5F9] border border-[#CBD5E0]" />
            </div>
          </div>
          {/* status */}
          <div className="tems-center gap-4">
            <div className="flex flex-col space-y-2">
            <Label htmlFor="name" className="text-right text-lg">
              Select Game Status
            </Label>
            <Input id="name" placeholder="Select" className="col-span-3 shadow-none text-gray-400 font-thin text-sm tracking-wider font-pincuk h-14 bg-[#F1F5F9] border border-[#CBD5E0]" />
            </div>
          </div>
        </div>
        <div className="flex gap-3 justify-end px-2"> 
          <SheetClose asChild>
            <Button type="submit" className="w-20 h-12 text-[#334154] bg-[#F8FAFC] border border-[#E2E8F0] hover:bg-accent">Reset</Button>
          </SheetClose>
          <SheetClose asChild>
            <Button type="submit" className="w-20 h-12 bg-[#D946EF] dark:text-white hover:text-[#D946EF] hover:bg-[#F3E8FF]">Filter</Button>
          </SheetClose>
        </div>
      </SheetContent>
    </Sheet>
  )
}

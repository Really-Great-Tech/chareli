...
export function CreateGameSheetUppy({
  children,
  onOpenChange,
}: {
  ...
  const handleSubmit = async (
    ...
    try {
      const result = await uploadGame(values.thumbnailFile, values.gameFile, {
        title: values.title,
              ...
              <div className="flex gap-3 justify-end px-2 mt-4">
                <SheetClose asChild>
                  <Button
                    type="button"
                    disabled={isUploading}
                    className="w-24 h-12 text-[#334154] bg-[#F8FAFC] border border-[#E2E8F0] hover:bg-[#E2E8F0] dark:text-gray-300 dark:bg-[#1E293B] dark:border-[#334155] dark:hover:bg-[#334155] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    onClick={() => {
                      formikRef.current?.resetForm();
                      setThumbnailPreview(null);
                      setGameFileName(null);
                ...
                <Button
                  ...
                  className="w-24 h-12 bg-[#D946EF] text-white hover:bg-[#C026D3] dark:text-white dark:hover:bg-[#C026D3] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >

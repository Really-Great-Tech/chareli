...
const PopularGamesConfiguration = forwardRef<PopularGamesConfigurationRef, PopularGamesConfigurationProps>(
  ({ disabled = false }, ref) => {
    const [isManualMode, setIsManualMode] = useState(false);
    const [selectedGameIds, setSelectedGameIds] = useState<string[]>([]);
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    
    const { data: popularConfigData, isLoading: isLoadingConfig } = useSystemConfigByKey('popular_games_settings');
    const { data: allGamesData, isLoading: isLoadingGames } = useGames({ status: 'active' });
    ...
    const selectedGames = allGames.filter(game => selectedGameIds.includes(game.id));
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative w-full max-w-md">
                    <button
                      ...
                      className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-left text-sm text-gray-900 dark:text-white disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-between hover:border-[#C17600] focus:border-[#C17600] focus:outline-none"
                    >
                  ...
                  {selectedGames.length === 0 && (
                    <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                      No games selected. Use the dropdown above to select games for the popular section.
                    </div>
                  )}
                </div>
              )}
